...
from mcp.db.models import WorkflowDefinition, WorkflowRun, MCP
from mcp.schemas.workflow import WorkflowCreate, WorkflowStep, WorkflowStepInput
from mcp.core.types import MCPType, InputSourceType

@pytest.fixture
def multi_step_workflow(test_app_client: TestClient, jwt_headers: Dict[str, str], test_db_session: Session):
...
def test_execute_workflow_with_fallback_workflow(
    ...
    response = test_app_client.post("/workflows/", json=fallback_data, headers=jwt_headers)
    assert response.status_code == 201
