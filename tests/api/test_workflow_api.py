...
from unittest import \
    mock  # For mocker.patch if not using pytest-mock directly for all patches
...
from mcp.db.models import MCP as MCPModel  # DB Models
from mcp.db.models import MCPVersion as MCPVersionModel
from mcp.db.models import WorkflowDefinition as WorkflowDefinitionModel
...
from mcp.schemas.workflow import WorkflowCreate as WorkflowCreateSchema
from mcp.schemas.workflow import WorkflowStep
...
def dummy_mcp_def(test_db_session: Session) -> MCPModel:
    ...
    from mcp.core import \
        registry as mcp_registry_service  # Ensure this import is correct

    db_mcp = mcp_registry_service.save_mcp_definition_to_db(
        db=test_db_session, mcp_data=mcp_create
    )
    return db_mcp
...
@pytest.fixture
def created_workflow_definition(
    test_db_session: Session, dummy_mcp_id: str
    ...
    workflow_payload = WorkflowCreateSchema(
        ...
        steps=[
            WorkflowStep(
                ...
                inputs={
                    "param1": WorkflowStepInput(
                        source_type=InputSourceType.STATIC_VALUE,
                        value="value1_from_db_wf",
                    )
                },
...
@pytest.fixture
def constraints_require_tag_prod(
    test_db_session: Session,
) -> ArchitecturalConstraints:  # Added session for potential future use
    """Constraints requiring all MCPs to have the 'prod' tag."""
    return ArchitecturalConstraints(required_tags_all_steps=["prod"])


@pytest.fixture
def constraints_prohibit_tag_experimental(
    test_db_session: Session,
) -> ArchitecturalConstraints:  # Added session
...
def test_create_workflow_success(
        ...
        "steps": [
            {
                ...
                "inputs": {
                    "data": {"source_type": "static_value", "value": "input_data_db"}
    ...
    # Verify in DB
    wf_from_db = (
        test_db_session.query(WorkflowDefinitionModel)
        .filter(WorkflowDefinitionModel.workflow_id == uuid.UUID(data["workflow_id"]))
        .first()
    )
...
def test_list_workflows_with_data(
    ...
    assert len(data) >= 1
    assert any(
        wf["workflow_id"] == str(created_workflow_definition.workflow_id) for wf in data
    )
...
def test_get_workflow_success(
    ...
    response = test_app_client.get(
        f"/workflows/{created_workflow_definition.workflow_id}", headers=jwt_headers
    )
    assert response.status_code == 200
    data = response.json()
        ...
        "steps": [
            {
                ...
                "mcp_version_id": "1.0.0",
                "inputs": {
                    "param1": {"source_type": "static_value", "value": "new_db_value"}
                },  # Corrected input
            }
        ],
    }
    response = test_app_client.put(
        f"/workflows/{workflow_id}", json=updated_data, headers=jwt_headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_delete_workflow_success(
    ...
    db_workflow = (
        test_db_session.query(WorkflowDefinitionModel)
        .filter(WorkflowDefinitionModel.workflow_id == uuid.UUID(workflow_id))
        .first()
    )
...
def test_execute_workflow_success_db_backed(
    ...
    str(created_workflow_definition.workflow_id)
    initial_inputs = {
        "initial_param": "start_value_for_db_exec"
    }  # This matches the input key in created_workflow_definition
    ...
    response_create = test_app_client.post(
        "/workflows/", json=exec_wf_payload.model_dump(mode="json"), headers=jwt_headers
    )
    ...
    response = test_app_client.post(
        f"/workflows/{exec_workflow_id}/execute",
    ...
    assert step_result["outputs_generated"] == {
        "output": "Processed: Dynamic Value for Execution"
    }
    ...
    run_entry = (
        test_db_session.query(WorkflowRun)
        .filter(WorkflowRun.workflow_id == uuid.UUID(exec_workflow_id))
        .first()
    )
...
def test_execute_workflow_step_mcp_def_not_found_in_db(
    ...
    response_create = test_app_client.post(
        "/workflows/", json=wf_payload.model_dump(mode="json"), headers=jwt_headers
    )
    assert (
        response_create.status_code == 201
    )  # Workflow definition creation should succeed
    exec_workflow_id = response_create.json()["workflow_id"]
    ...
    )  # The API call itself is okay, the workflow execution fails
    data = response.json()
    ...
    assert (
        len(data["step_results"]) == 0
    )  # Should fail before any step execution attempt if MCP can't be loaded
...
def test_execute_workflow_step_mcp_version_not_found(
    ...
    assert data["workflow_id"] == exec_workflow_id
    assert (
        f"MCP instance for ID '{existing_mcp_id}' (Version: {non_existent_mcp_version_id}) not found or failed to instantiate."
        in data["error_message"]
    )
...
def test_execute_workflow_mcp_instantiation_failure_bad_config(
    ...
    mcp_version_entry.config_snapshot = {
        "invalid_config_key": 12345
    }  # This is not PythonScriptConfig
    ...
    assert (
        f"MCP instance for ID '{mcp_id}' (Version: {mcp_version_str}) not found or failed to instantiate."
        in data["error_message"]
    )
    assert len(data["step_results"]) == 0
...
def mock_constraints_in_api(
    mocker, constraints_obj: Optional[ArchitecturalConstraints]
):
    ...
    mock_model_validate = mocker.patch(
        "mcp.schemas.mcd_constraints.ArchitecturalConstraints.model_validate"
    )
    ...
    if constraints_obj is not None:
        ...
        mock_model_validate.return_value = (
            None  # or have it raise an error if that's tested elsewhere
        )
...
def test_execute_workflow_fails_max_steps_constraint(
    ...
    wf_def = (
        ...
        .filter(
            ...
            == created_workflow_definition.workflow_id
        )
        .first()
    )
    ...
    response = test_app_client.post(
        f"/workflows/{wf_def.workflow_id}/execute", json={}, headers=jwt_headers
    )

    assert (
        response.status_code == 400
    )  # Expecting 400 due to ValueError from WorkflowEngine
...
def test_execute_workflow_fails_prohibited_type_constraint(
    ...
    mcp_to_update = (
        test_db_session.query(MCPModel).filter(MCPModel.id == dummy_mcp_def.id).first()
    )
    ...
    response = test_app_client.post(
        f"/workflows/{created_workflow_definition.workflow_id}/execute",
        json={},
        headers=jwt_headers,
    )
