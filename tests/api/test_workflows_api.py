...
@pytest.fixture(scope="function") # Changed to function scope for cleaner state if tests modify it
def ensure_passthrough_mcp_exists(client: TestClient):
    ...
    if SAMPLE_MCP_ID_VALID not in mcp_server_registry:
        ...
        from mcp.core.python_script import PythonScriptMCP, PythonScriptConfig
        mock_config_data = {
...
@pytest.fixture
def create_sample_workflow(client: TestClient, sample_workflow_payload: Dict[str, Any]) -> Workflow:
    """Helper fixture to pre-populate a workflow for tests that need one."""
    response = client.post("/workflows/", json=sample_workflow_payload, headers=get_headers())
...
def test_create_workflow_no_api_key(client: TestClient, sample_workflow_payload: Dict[str, Any]):
    response = client.post("/workflows/", json=sample_workflow_payload)
    assert response.status_code == 403 # Expecting 403 due to auto_error=True in APIKeyHeader
    assert "Not authenticated" in response.json().get("detail", "").lower() or \
           "api key" in response.json().get("detail", "").lower() # More flexible check for auth error messages

def test_create_workflow_invalid_api_key(client: TestClient, sample_workflow_payload: Dict[str, Any]):
    response = client.post("/workflows/", json=sample_workflow_payload, headers={"X-API-Key": "invalidkey"})
    assert response.status_code == 403
    assert "invalid api key" in response.json().get("detail", "").lower()

# === POST /workflows/ ===
def test_create_workflow_success(client: TestClient, sample_workflow_payload: Dict[str, Any]):
    response = client.post("/workflows/", json=sample_workflow_payload, headers=get_headers())
    assert response.status_code == 201
    data = response.json()
...
def test_create_workflow_invalid_mcp_id(client: TestClient, sample_workflow_payload: Dict[str, Any]):
    payload_invalid_mcp = sample_workflow_payload.copy()
    payload_invalid_mcp["steps"][0]["mcp_id"] = SAMPLE_MCP_ID_INVALID
    response = client.post("/workflows/", json=payload_invalid_mcp, headers=get_headers())
    assert response.status_code == 400
    assert f"mcp with id {SAMPLE_MCP_ID_INVALID} not found" in response.json()["detail"].lower()

def test_create_workflow_bad_input_source_config(client: TestClient, sample_workflow_payload: Dict[str, Any]):
    ...
    assert any("workflow_input_key' must be provided" in err.get("msg", "") for err in details if isinstance(err, dict))
...
def test_update_workflow_success(client: TestClient, create_sample_workflow: Workflow, sample_workflow_payload: Dict[str, Any]):
    workflow_id = create_sample_workflow.workflow_id
    ...
    response = client.put(f"/workflows/{workflow_id}", json=update_payload, headers=get_headers())
    assert response.status_code == 200
    data = response.json()
    assert data["workflow_id"] == workflow_id
    assert data["name"] == "Updated Workflow Name"
    assert data["description"] == "Updated description."
    assert data["created_at"] == create_sample_workflow.created_at.isoformat().replace("+00:00", "Z") # Ensure ISO format matches
    ...
    parsed_updated_at = datetime.fromisoformat(data["updated_at"].replace("Z", "+00:00"))
    assert parsed_updated_at > original_updated_at 

def test_update_workflow_not_found(client: TestClient, sample_workflow_payload: Dict[str, Any]):
    non_existent_id = str(uuid.uuid4())
    response = client.put(f"/workflows/{non_existent_id}", json=sample_workflow_payload, headers=get_headers())
    assert response.status_code == 404

def test_update_workflow_invalid_mcp_id(client: TestClient, create_sample_workflow: Workflow, sample_workflow_payload: Dict[str, Any]):
    workflow_id = create_sample_workflow.workflow_id
    update_payload = sample_workflow_payload.copy()
