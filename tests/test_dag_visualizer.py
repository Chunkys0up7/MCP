...
def sample_workflow() -> WorkflowDefinition:
    ...
    return WorkflowDefinition(
        ...
        steps=[
            WorkflowStep(
                step_id="step1", name="Step 1", depends_on=[], config={}, mcp_id="mcp1"
            ),
            WorkflowStep(
                step_id="step2",
                name="Step 2",
...
def test_visualize(engine_with_execution: DAGWorkflowEngine, tmp_path):
    ...
    visualizer.visualize(
        engine_with_execution, output_path=str(output_path), show=False
    )
...
def test_get_parallel_steps(engine_with_execution: DAGWorkflowEngine):
    ...
    assert set(["step2", "step3"]) in [
        set(group) for group in parallel_groups
    ]  # Steps 2 and 3 should be parallel
