...
def test_workflow_analysis():
    ...
    # Verify suggestions
    assert any(
        s.type == SuggestionType.SCHEMA_REPAIR for s in suggestions
    ), "Should detect schema issues"
    assert any(
        s.type == SuggestionType.BEST_PRACTICE for s in suggestions
    ), "Should detect best practice issues"
...
def test_error_resolution():
    ...
    # Verify suggestions
    assert all(
        s.type == SuggestionType.ERROR_RESOLUTION for s in suggestions
    ), "Should provide error resolution suggestions"
