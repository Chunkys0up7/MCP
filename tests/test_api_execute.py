...
def test_llm_prompt_execution():
    ...
    try:
        ...
        execute_resp = requests.post(
            f"{API_BASE_URL}/context/{server_id}/execute", json=inputs, headers=HEADERS
        )
        assert (
            execute_resp.status_code == 200
        ), f"Failed to execute MCP: {execute_resp.text}"
        ...
        delete_resp = requests.delete(
            f"{API_BASE_URL}/context/{server_id}", headers=HEADERS
        )
        assert (
            delete_resp.status_code == 200
        ), f"Failed to delete MCP: {delete_resp.text}"
