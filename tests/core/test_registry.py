...
from mcp.db.models import MCP as MCPModel, MCPVersion as MCPVersionModel, EMBEDDING_DIM
from mcp.schemas.mcp import MCPCreate as MCPCreateSchema, MCPUpdate as MCPUpdateSchema
...
def test_save_mcp_definition_to_db_success(test_db_session: Session, basic_mcp_create_payload: MCPCreateSchema):
    created_mcp = mcp_registry_service.save_mcp_definition_to_db(db=test_db_session, mcp_data=basic_mcp_create_payload)
    ...
    assert version.config_snapshot["script_content"] == basic_mcp_create_payload.initial_config["script_content"]

    # Verify it's actually in the DB
    retrieved_mcp = test_db_session.query(MCPModel).filter(MCPModel.id == created_mcp.id).first()
    assert retrieved_mcp is not None
    assert retrieved_mcp.name == basic_mcp_create_payload.name

def test_save_mcp_definition_to_db_invalid_config(test_db_session: Session, basic_mcp_create_payload: MCPCreateSchema):
    invalid_config_payload = basic_mcp_create_payload.model_copy()
    invalid_config_payload.initial_config = {"wrong_key": "for_python_script"} # Invalid for PythonScriptConfig
    
    with pytest.raises(ValueError, match="Invalid initial_config for MCP type python_script"):
        mcp_registry_service.save_mcp_definition_to_db(db=test_db_session, mcp_data=invalid_config_payload)

@patch('mcp.core.registry.embedding_model', None) # Simulate embedding model not loading
def test_save_mcp_definition_to_db_no_embedding_model(test_db_session: Session, basic_mcp_create_payload: MCPCreateSchema):
    created_mcp = mcp_registry_service.save_mcp_definition_to_db(db=test_db_session, mcp_data=basic_mcp_create_payload)
    assert created_mcp.embedding is None

# === Tests for load_mcp_definition_from_db ===
def test_load_mcp_definition_from_db_found(test_db_session: Session, basic_mcp_create_payload: MCPCreateSchema):
    created_mcp = mcp_registry_service.save_mcp_definition_to_db(db=test_db_session, mcp_data=basic_mcp_create_payload)
    
    loaded_mcp = mcp_registry_service.load_mcp_definition_from_db(db=test_db_session, mcp_id_str=str(created_mcp.id))
...
def test_load_mcp_definition_from_db_not_found(test_db_session: Session):
    ...
    loaded_mcp = mcp_registry_service.load_mcp_definition_from_db(db=test_db_session, mcp_id_str=non_existent_id)
    assert loaded_mcp is None

def test_load_mcp_definition_from_db_invalid_uuid(test_db_session: Session):
    loaded_mcp = mcp_registry_service.load_mcp_definition_from_db(db=test_db_session, mcp_id_str="not-a-uuid")
    assert loaded_mcp is None
...
def test_load_all_mcp_definitions_from_db_with_data(test_db_session: Session, basic_mcp_create_payload: MCPCreateSchema):
    mcp1_payload = basic_mcp_create_payload.model_copy()
    mcp1_payload.name = "MCP One"
    mcp_registry_service.save_mcp_definition_to_db(db=test_db_session, mcp_data=mcp1_payload)
    
    mcp2_payload = basic_mcp_create_payload.model_copy()
    mcp2_payload.name = "MCP Two"
    mcp_registry_service.save_mcp_definition_to_db(db=test_db_session, mcp_data=mcp2_payload)
...
def test_update_mcp_definition_in_db_success(test_db_session: Session, basic_mcp_create_payload: MCPCreateSchema):
    created_mcp = mcp_registry_service.save_mcp_definition_to_db(db=test_db_session, mcp_data=basic_mcp_create_payload)
    ...
    updated_mcp = mcp_registry_service.update_mcp_definition_in_db(db=test_db_session, mcp_id_str=str(created_mcp.id), mcp_data=update_payload)
    ...
    assert updated_mcp.type == basic_mcp_create_payload.type.value # Type should not change
    
    # Check if embedding was updated
    assert updated_mcp.embedding is not None
    assert updated_mcp.embedding != original_embedding # Assuming name/desc/tags change leads to new embedding
    assert len(updated_mcp.embedding) == EMBEDDING_DIM

def test_update_mcp_definition_in_db_no_change_no_embedding_update(test_db_session: Session, basic_mcp_create_payload: MCPCreateSchema):
    created_mcp = mcp_registry_service.save_mcp_definition_to_db(db=test_db_session, mcp_data=basic_mcp_create_payload)
    ...
    update_payload = MCPUpdateSchema(name=created_mcp.name) # No actual change to name, desc, or tags
...
def test_update_mcp_definition_in_db_not_found(test_db_session: Session):
    ...
    updated_mcp = mcp_registry_service.update_mcp_definition_in_db(db=test_db_session, mcp_id_str=non_existent_id, mcp_data=update_payload)
    assert updated_mcp is None

# === Tests for delete_mcp_definition_from_db ===
def test_delete_mcp_definition_from_db_success(test_db_session: Session, basic_mcp_create_payload: MCPCreateSchema):
    created_mcp = mcp_registry_service.save_mcp_definition_to_db(db=test_db_session, mcp_data=basic_mcp_create_payload)
    mcp_id = created_mcp.id
    assert test_db_session.query(MCPModel).filter(MCPModel.id == mcp_id).first() is not None

    deleted = mcp_registry_service.delete_mcp_definition_from_db(db=test_db_session, mcp_id_str=str(mcp_id))
    ...
    assert test_db_session.query(MCPVersionModel).filter(MCPVersionModel.mcp_id == mcp_id).count() == 0

def test_delete_mcp_definition_from_db_not_found(test_db_session: Session):
    non_existent_id = str(uuid.uuid4())
    deleted = mcp_registry_service.delete_mcp_definition_from_db(db=test_db_session, mcp_id_str=non_existent_id)
...
def test_generate_mcp_embedding_with_mcp_create(mock_embedding_model_global, basic_mcp_create_payload: MCPCreateSchema):
    ...
    expected_text = "Test Registry MCP A test MCP for registry functions. registry python"
...
def test_generate_mcp_embedding_with_mcp_update(mock_embedding_model_global):
    ...
    existing_mcp_mock = MCPModel(id=uuid.uuid4(), name="Original Name", description="Original Desc", tags=["orig_tag"])

    embedding = mcp_registry_service._generate_mcp_embedding(update_payload, existing_mcp=existing_mcp_mock)
    assert embedding is not None
...
def test_generate_mcp_embedding_empty_input():
    empty_payload = MCPCreateSchema(
        name="", type=MCPType.PYTHON_SCRIPT, initial_version_str="1", initial_config={"script_content":""}, 
        description="", tags=[]
    )
...
@patch('mcp.core.registry.embedding_model')
def test_search_mcp_definitions_by_text_success(mock_embedding_model_global, test_db_session: Session):
    mock_encoder = MagicMock()
    ...
    mock_db_mcps = [
        MCPModel(id=uuid.uuid4(), name="Search Result 1", embedding=[0.31]*EMBEDDING_DIM),
        MCPModel(id=uuid.uuid4(), name="Search Result 2", embedding=[0.32]*EMBEDDING_DIM)
    ]
    ...
    with patch.object(test_db_session, 'query') as mock_db_query:
        ...
        results = mcp_registry_service.search_mcp_definitions_by_text(test_db_session, "test query", limit=5)
...
def test_search_mcp_definitions_by_text_no_model(test_db_session: Session):
    results = mcp_registry_service.search_mcp_definitions_by_text(test_db_session, "test query")
    assert results == []

def test_search_mcp_definitions_by_text_empty_query(test_db_session: Session):
    results = mcp_registry_service.search_mcp_definitions_by_text(test_db_session, "   ")
    assert results == []
