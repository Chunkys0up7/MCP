...
from mcp.schemas.workflow import Workflow, WorkflowStep, ErrorHandlingConfig, InputSourceType
...
from mcp.db.models import MCP as MCPModel # Renaming to avoid clash if MCP schema is also imported directly
...
def create_mock_mcp_model(mcp_id: str, mcp_type: MCPType, tags: list[str] | None = None) -> MCPModel:
    mock_model = MagicMock(spec=MCPModel)
    try:
        mock_model.id = uuid.UUID(mcp_id)
    except ValueError:
        print(f"Warning: Invalid mcp_id '{mcp_id}' passed to create_mock_mcp_model. Generating a new UUID.")
...
async def test_workflow_engine_instantiation(mock_db_session, allow_all_constraints):
    engine = WorkflowEngine(db_session=mock_db_session, constraints=allow_all_constraints)
...
async def test_run_workflow_successful_sequential_execution(
    ...
    with patch('mcp.core.registry.load_mcp_definition_from_db') as mock_load_mcp_def:
        mock_mcp_model_for_validation = create_mock_mcp_model(
            mcp_id=basic_workflow_definition.steps[0].mcp_id, mcp_type=MCPType.PYTHON_SCRIPT
        )
        ...
        mock_mcp_instance = MockMCPServer(
            config=MockMCPConfig(setting="test", name="TestMCP1", type=MCPType.PYTHON_SCRIPT)
        )
...
async def test_run_workflow_step_failure_stop_on_error(
    ...
    with patch('mcp.core.registry.load_mcp_definition_from_db') as mock_load_mcp_def:
        mock_mcp_model_for_validation = create_mock_mcp_model(mcp_id=basic_workflow_definition.steps[0].mcp_id, mcp_type=MCPType.PYTHON_SCRIPT)
        mock_load_mcp_def.return_value = mock_mcp_model_for_validation

        engine = WorkflowEngine(db_session=mock_db_session, constraints=allow_all_constraints)

        mock_mcp_instance = MockMCPServer(
            config=MockMCPConfig(setting="test", name="TestMCPFail", type=MCPType.PYTHON_SCRIPT)
        )
        mock_mcp_instance.execute = AsyncMock(
...
async def test_run_workflow_mcp_instantiation_failure(
    ...
    with patch('mcp.core.registry.load_mcp_definition_from_db') as mock_load_mcp_def:
        ...
        assert result.status == "FAILED"
        assert result.error_message is not None
        assert "MCP instance for ID '{}' (Version: 1.0.0) not found or failed to instantiate.".format(basic_workflow_definition.steps[0].mcp_id) in result.error_message
        assert len(result.step_results) == 1 
        assert result.step_results[0]["status"] == "FAILED"
        assert basic_workflow_definition.steps[0].mcp_id in result.step_results[0]["error"]
        assert result.final_outputs is None
...
async def test_run_workflow_two_steps_data_passing(
    ...
    with patch('mcp.core.registry.load_mcp_definition_from_db') as mock_load_mcp_def:
        ...
        mock_mcp_instance_1 = MockMCPServer(
            config=MockMCPConfig(setting="test1", name="MCP1", type=MCPType.PYTHON_SCRIPT)
        )
        ...
        mock_mcp_instance_2 = MockMCPServer(
            config=MockMCPConfig(setting="test2", name="MCP2", type=MCPType.PYTHON_SCRIPT)
        )
        ...
        def side_effect_load_def(db, mcp_id_str):
            if mcp_id_str == mock_mcp_id_1:
                return create_mock_mcp_model(mcp_id=mock_mcp_id_1, mcp_type=MCPType.PYTHON_SCRIPT)
            elif mcp_id_str == mock_mcp_id_2:
                return create_mock_mcp_model(mcp_id=mock_mcp_id_2, mcp_type=MCPType.PYTHON_SCRIPT)
            return None
        ...
        mock_get_mcp_instance.assert_any_call(db=mock_db_session, mcp_id_str=mock_mcp_id_1, mcp_version_str="1.0")
        mock_get_mcp_instance.assert_any_call(db=mock_db_session, mcp_id_str=mock_mcp_id_2, mcp_version_str="1.0")
...
def test_validate_constraints_max_steps_violation(
    ...
    two_step_workflow.steps.append(WorkflowStep(
        step_id="step-2", mcp_id=mock_mcp_id_2, name="Mock Step 2", inputs={}, # MODIFIED: use mock_mcp_id_2
        mcp_version_id="1.0.0"
    ))
    
    engine = WorkflowEngine(db_session=mock_db_session, constraints=restrictive_constraints_max_steps)
...
def test_validate_constraints_allowed_type_violation(
    ...
    mock_mcp_model = create_mock_mcp_model(mcp_id=basic_workflow_definition.steps[0].mcp_id, mcp_type=MCPType.LLM_PROMPT)
    mock_load_mcp_def.return_value = mock_mcp_model
    
    engine = WorkflowEngine(db_session=mock_db_session, constraints=restrictive_constraints_allowed_types)
    ...
    expected_allowed_str = str([m.value for m in restrictive_constraints_allowed_types.allowed_mcp_types])
    assert expected_allowed_str in str(excinfo.value)
    mock_load_mcp_def.assert_called_once_with(db=mock_db_session, mcp_id_str=basic_workflow_definition.steps[0].mcp_id)
...
def test_validate_constraints_prohibited_type_violation(
    ...
    engine = WorkflowEngine(db_session=mock_db_session, constraints=restrictive_constraints_prohibited_types)
    
    with pytest.raises(ValueError) as excinfo:
        engine._validate_workflow_against_constraints(basic_workflow_definition)
    assert "is in the list of prohibited types" in str(excinfo.value)
    assert f"type '{MCPType.LLM_PROMPT.value}'" in str(excinfo.value)
    expected_prohibited_str = str([m.value for m in restrictive_constraints_prohibited_types.prohibited_mcp_types])
...
def test_validate_constraints_required_tag_missing(
    ...
    mock_mcp_model = create_mock_mcp_model(mcp_id=basic_workflow_definition.steps[0].mcp_id, mcp_type=MCPType.PYTHON_SCRIPT, tags=["dev"])
    mock_load_mcp_def.return_value = mock_mcp_model
    
    engine = WorkflowEngine(db_session=mock_db_session, constraints=restrictive_constraints_required_tags)
...
def test_validate_constraints_prohibited_tag_present(
    ...
    mock_mcp_model = create_mock_mcp_model(mcp_id=basic_workflow_definition.steps[0].mcp_id, mcp_type=MCPType.PYTHON_SCRIPT, tags=["experimental", "dev"])
    mock_load_mcp_def.return_value = mock_mcp_model
    
    engine = WorkflowEngine(db_session=mock_db_session, constraints=restrictive_constraints_prohibited_tags)
...
def test_validate_constraints_mcp_def_not_found(
    ...
    with pytest.raises(ValueError) as excinfo:
        engine._validate_workflow_against_constraints(basic_workflow_definition)
    assert "MCP definition for ID '{}' in step 'Mock Step 1' not found.".format(basic_workflow_definition.steps[0].mcp_id) in str(excinfo.value)
...
def test_validate_constraints_successful_with_constraints(
    ...
    mock_mcp_model = create_mock_mcp_model(mcp_id=basic_workflow_definition.steps[0].mcp_id, mcp_type=MCPType.PYTHON_SCRIPT, tags=[])
    mock_load_mcp_def.return_value = mock_mcp_model
    ...
    try:
        ...
        pytest.fail("_validate_workflow_against_constraints raised ValueError unexpectedly")
...
@pytest.mark.asyncio
@patch('mcp.core.registry.load_mcp_definition_from_db')
@patch('mcp.core.registry.get_mcp_instance_from_db') # Also mock this, though it shouldn't be called if validation fails
async def test_run_workflow_fails_on_constraint_violation(
    ...
    # Simulate mcp-mock-001 is of type LLM_PROMPT, which is prohibited
    mock_mcp_model_for_validation = create_mock_mcp_model(mcp_id=basic_workflow_definition.steps[0].mcp_id, mcp_type=MCPType.LLM_PROMPT)
    mock_load_mcp_def.return_value = mock_mcp_model_for_validation
    ...
    assert result.status == "FAILED"
    assert result.error_message is not None
    assert "is in the list of prohibited types" in result.error_message # Error from validation
