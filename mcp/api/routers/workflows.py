...
from mcp.schemas.workflow import \
    WorkflowExecutionResult as \
    WorkflowExecutionResultSchema  # For validating steps
from mcp.schemas.workflow import WorkflowStepGantt
...
async def create_workflow_definition(
    ...
    try:
        db_workflow = workflow_registry_service.save_workflow_definition_to_db(
            db=db, workflow_data=workflow_data
        )
        log_audit_action(
            db,
            user_id=current_user_sub,
            action_type="create_workflow",
            target_id=db_workflow.workflow_id,
            details=workflow_data.dict(),
        )
        return db_workflow
...
async def update_workflow_definition(
    ...
    try:
        db_workflow = workflow_registry_service.update_workflow_definition_in_db(
            db=db, workflow_id_str=workflow_id, workflow_data=workflow_data
        )
        if db_workflow is None:
            raise HTTPException(status_code=404, detail="Workflow definition not found")
        log_audit_action(
...
async def delete_workflow_definition(
    ...
    try:
        success = workflow_registry_service.delete_workflow_definition_from_db(
            db=db, workflow_id_str=workflow_id
        )
        if not success:
            raise HTTPException(status_code=404, detail="Workflow definition not found")
        log_audit_action(
...
async def execute_workflow(
    workflow_id: str,
    initial_inputs: Optional[Dict[str, Any]] = Body(
        None, description="Initial inputs for the workflow"
    ),
    db: Session = Depends(get_db_session),
    current_user_sub: str = Depends(get_current_subject),
    _: List[str] = Depends(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to create workflow run record: {str(e)}"
        )
    ...
    try:
        execution_result = await workflow_engine.run_workflow(
            workflow_schema_for_engine, initial_inputs
        )
...
@router.get(
    "/runs/{run_id}", response_model=WorkflowExecutionResultSchema
)  # This might need a more DB-aligned schema
...
@router.get(
    "/runs/", response_model=List[WorkflowExecutionResultSchema]
)  # Again, schema might need adjustment
...
@router.get("/runs/{run_id}/gantt", response_model=List[WorkflowStepGantt])
async def get_workflow_run_gantt(
    run_id: str,
    db: Session = Depends(get_db_session),
    current_user_sub: str = Depends(get_current_subject),
):
