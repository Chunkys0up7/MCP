...
MCP_CLIENT_SPECIFIC_LOGGER.debug(
    f"MCP_CLIENT_SPECIFIC_LOGGER initialized. Type: {type(MCP_CLIENT_SPECIFIC_LOGGER)}, ID: {id(MCP_CLIENT_SPECIFIC_LOGGER)}"
)
...
class MCPClient:
    ...
    def __init__(self, base_url: Optional[str] = None, api_key: Optional[str] = None):
        ...
        MCP_CLIENT_SPECIFIC_LOGGER.debug(
            f"MCPClient instance created. Logger type: {type(MCP_CLIENT_SPECIFIC_LOGGER)}, ID: {id(MCP_CLIENT_SPECIFIC_LOGGER)}"
        )
    ...
    def _handle_response(self, response: requests.Response) -> Dict[str, Any]:
        ...
        MCP_CLIENT_SPECIFIC_LOGGER.debug(
            f"_handle_response received response with status: {response.status_code} from URL: {response.url}"
        )
        if response.status_code == 404:
            MCP_CLIENT_SPECIFIC_LOGGER.error(
                f"MCPNotFoundError for URL: {response.url}"
            )
            raise MCPNotFoundError(f"Resource not found: {response.url}")
        elif response.status_code == 400:
            MCP_CLIENT_SPECIFIC_LOGGER.error(
                f"MCPValidationError for URL: {response.url}. Response text: {response.text}"
            )
            raise MCPValidationError(f"Validation error: {response.text}")
        elif response.status_code >= 400:
            MCP_CLIENT_SPECIFIC_LOGGER.error(
                f"MCPAPIError for URL: {response.url}. Status: {response.status_code}. Response text: {response.text}"
            )
            raise MCPAPIError(f"API error: {response.text}")

        try:
            MCP_CLIENT_SPECIFIC_LOGGER.debug(
                f"Type of 'response' object in _handle_response BEFORE .json() call: {type(response)}"
            )
            MCP_CLIENT_SPECIFIC_LOGGER.debug(
                f"_handle_response raw response text: {response.text}"
            )
            json_response = response.json()
            try:
                MCP_CLIENT_SPECIFIC_LOGGER.debug(
                    f"_handle_response: successfully parsed JSON. Type of json_response: {type(json_response)}"
                )
            except AttributeError as ae_type_log:
                MCP_CLIENT_SPECIFIC_LOGGER.error(
                    f"AttributeError SPECIFICALLY WHEN LOGGING type(json_response): {str(ae_type_log)}"
                )
                MCP_CLIENT_SPECIFIC_LOGGER.error(
                    f"Type of json_response at this point was: {type(json_response)}"
                )
                MCP_CLIENT_SPECIFIC_LOGGER.error(
                    f"Content of json_response: {json_response}"
                )
                raise

            MCP_CLIENT_SPECIFIC_LOGGER.debug(
                f"_handle_response content being returned: {json_response}"
            )
            return json_response
        except requests.exceptions.JSONDecodeError as e:
            MCP_CLIENT_SPECIFIC_LOGGER.error(
                f"JSONDecodeError in _handle_response for URL: {response.url}. Error: {str(e)}. Response text: {response.text}"
            )
            raise MCPAPIError(
                f"Failed to decode JSON response from {response.url}: {str(e)}. Response text: {response.text}"
            )
        except (
            AttributeError
        ) as ae_general:  # Catch any other AttributeErrors in this block
            MCP_CLIENT_SPECIFIC_LOGGER.error(
                f"A GENERAL AttributeError occurred in _handle_response try block: {str(ae_general)}"
            )
            MCP_CLIENT_SPECIFIC_LOGGER.error(
    ...
    def get_servers(self) -> List[Dict[str, Any]]:
        ...
        MCP_CLIENT_SPECIFIC_LOGGER.debug(
            f"get_servers attempting to GET from URL: {url}. Logger type: {type(MCP_CLIENT_SPECIFIC_LOGGER)}, ID: {id(MCP_CLIENT_SPECIFIC_LOGGER)}"
        )
        try:
            response = requests.get(url, headers=self.headers)
            MCP_CLIENT_SPECIFIC_LOGGER.debug(
                f"get_servers received response. Status: {response.status_code}, URL: {response.url}"
            )
        except requests.exceptions.RequestException as e:
            MCP_CLIENT_SPECIFIC_LOGGER.error(
                f"RequestException in get_servers for URL: {url}. Error: {str(e)}"
            )
            raise MCPAPIError(f"Request failed for {url}: {str(e)}")

        data = self._handle_response(response)
        MCP_CLIENT_SPECIFIC_LOGGER.debug(
            "get_servers received data from _handle_response."
        )
    ...
    def create_server(
        ...
        specific_config_dict: Dict[str, Any],
    ) -> Dict[str, Any]:
        """Create a new MCP server."""
        MCP_CLIENT_SPECIFIC_LOGGER.debug(
            f"create_server called for name: {name}, type: {mcp_type}. Logger type: {type(MCP_CLIENT_SPECIFIC_LOGGER)}, ID: {id(MCP_CLIENT_SPECIFIC_LOGGER)}"
        )
    ...
    def delete_server(self, server_id: str) -> bool:
        ...
        MCP_CLIENT_SPECIFIC_LOGGER.debug(
            f"delete_server called for ID: {server_id}. Logger type: {type(MCP_CLIENT_SPECIFIC_LOGGER)}, ID: {id(MCP_CLIENT_SPECIFIC_LOGGER)}"
        )
        try:
            response = requests.delete(
                f"{self.base_url}/context/{server_id}", headers=self.headers
            )
            if response.status_code == 204:  # No Content, successful deletion
                return True
            elif response.status_code == 404:
                MCP_CLIENT_SPECIFIC_LOGGER.error(
                    f"MCPNotFoundError for delete URL: {response.url}"
                )
                raise MCPNotFoundError(
                    f"Server ID '{server_id}' not found for deletion."
                )
            else:
                ...
                MCP_CLIENT_SPECIFIC_LOGGER.error(
                    f"MCPAPIError for delete URL: {response.url}. Status: {response.status_code}. Detail: {error_detail}"
                )
                raise MCPAPIError(
                    f"Failed to delete server '{server_id}'. Status: {response.status_code}. Error: {error_detail}"
                )
        except requests.exceptions.RequestException as e:
            MCP_CLIENT_SPECIFIC_LOGGER.error(
                f"RequestException in delete_server for ID {server_id}. Error: {str(e)}"
            )
            raise MCPAPIError(
                f"Request failed while trying to delete server '{server_id}': {str(e)}"
            )
...
if __name__ == "__main__":
    ...
    MCP_CLIENT_SPECIFIC_LOGGER.info(
        "MCPClient module run directly. This is for testing only."
    )
