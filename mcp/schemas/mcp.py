...
from mcp.core.types import \
    MCPType  # MCPType enum and the Union of all config types
...
class MCPBase(BaseModel):
    name: str = Field(
        ..., min_length=3, max_length=100, description="User-defined name for the MCP."
    )
...
class MCPDetail(
    ...
    latest_version_config: Optional[Dict[str, Any]] = Field(
        default=None, description="Configuration of the latest version of this MCP."
    )
    latest_version_str: Optional[str] = Field(
        default=None, description="Version string of the latest version."
    )
...
class MCPNewVersionRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None, description="Description for this new version."
    )
    config_snapshot: Dict[str, Any] = Field(
        ..., description="The full configuration snapshot for this new version."
    )
