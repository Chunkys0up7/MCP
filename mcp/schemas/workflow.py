...
class WorkflowStepInput(BaseModel):
    ...
    value: Optional[Any] = Field(default=None, description="The static value for the input. Required and must be set if source_type is STATIC_VALUE.")
    source_step_id: Optional[str] = Field(default=None, description="The ID of the step from which to pull the output. Required if source_type is STEP_OUTPUT.")
    source_output_name: Optional[str] = Field(default=None, description="The name of the output field from the source_step_id. Required if source_type is STEP_OUTPUT.")
    workflow_input_key: Optional[str] = Field(default=None, description="The key referencing an input provided at the start of the workflow execution. Required if source_type is WORKFLOW_INPUT.")
...
class ErrorHandlingConfig(BaseModel):
    ...
    max_retries: Optional[int] = Field(default=None, ge=0, description="Maximum number of retry attempts if strategy is 'Retry with Backoff'. Must be >= 0.")
    backoff_factor: Optional[float] = Field(default=None, ge=1.0, description="The factor by which the delay increases for each retry attempt if strategy is 'Retry with Backoff'. Must be >= 1.0.")
    fallback_workflow_id: Optional[str] = Field(default=None, description="The ID of another workflow to execute if the 'Fallback Chain' strategy is chosen and an error occurs.")
...
class WorkflowBase(BaseModel):
    ...
    description: Optional[str] = Field(default=None, description="An optional detailed description of what the workflow does.")
    ...
    error_handling: ErrorHandlingConfig = Field(default_factory=ErrorHandlingConfig, description="Configuration for error handling within the workflow.")
    # Consider adding:
...
class WorkflowCreate(WorkflowBase):
    ...
    """
    steps: List[WorkflowStep] = Field(default_factory=list, description="An ordered list of steps that constitute the workflow. For sequential execution, order matters. For future parallel/DAG execution, dependencies would be specified within steps.")
...
class Workflow(WorkflowBase):
    ...
    """
    workflow_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique identifier for the workflow, generated by the system.")
    steps: List[WorkflowStep] = Field(..., description="The list of steps that define the workflow's operations.")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp of when the workflow was created.")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp of when the workflow was last updated.")
...
class WorkflowExecutionResult(BaseModel):
    ...
    execution_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique ID for this specific execution instance of the workflow.")
    status: str = Field(..., examples=["PENDING", "RUNNING", "SUCCESS", "FAILED", "CANCELLED"], description="The overall status of the workflow execution.")
    started_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp of when the workflow execution began.")
    finished_at: Optional[datetime] = Field(default=None, description="Timestamp of when the workflow execution completed (either successfully or with failure).")
    step_results: List[Dict[str, Any]] = Field(default_factory=list, description="A list containing the results from each executed step. Each item in the list should ideally conform to a StepExecutionResult model (to be defined).")
    final_outputs: Optional[Dict[str, Any]] = Field(default=None, description="The final designated outputs of the workflow, if any. This could be an aggregation or specific step outputs.")
    error_message: Optional[str] = Field(default=None, description="Error message if the workflow execution failed.")
