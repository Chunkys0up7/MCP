...
class PythonScriptMCP(BaseMCPServer):
    ...
    def __init__(self, config: PythonScriptConfig):
        ...
        if self.config.script_content:
            ...
            try:
                ...
                logger.info(f"Created temporary script file for execution: {self._script_path_to_execute} from script_content of MCP '{self.config.name}'")
            except Exception as e:
                logger.error(f"Failed to create temporary script file from script_content for MCP '{self.config.name}': {e}")
    ...
    async def execute(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self._script_path_to_execute or not os.path.exists(self._script_path_to_execute):
            logger.error(f"Cannot execute PythonScriptMCP '{self.config.name}': Effective script path '{self._script_path_to_execute}' is invalid or missing.")
            return {"success": False, "result": None, "error": "Script path misconfigured or temporary script creation failed."}
        ...
        def _run_script_sync():
            ...
            try:
                with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False, encoding='utf-8') as tmp_input_f_obj:
                    json.dump(inputs, tmp_input_f_obj)
                    tmp_input_file_path = tmp_input_f_obj.name
                
                with tempfile.NamedTemporaryFile(mode="r", suffix=".json", delete=False, encoding='utf-8') as tmp_output_f_obj:
                    tmp_output_file_path = tmp_output_f_obj.name
                _temporary_script_files.add(tmp_output_file_path) # Ensure cleanup even if script fails before writing
                ...
                if process_return_code == 0:
                    if os.path.exists(tmp_output_file_path):
                        ...
                        logger.error(f"Script for '{self.config.name}' executed successfully (rc=0) but output file '{tmp_output_file_path}' was not found.")
                        return {"success": False, "result": None, "error": "Script executed (rc=0) but output file missing.", "stdout": stdout_str, "stderr": stderr_str}
                else:
                    logger.error(f"Script execution for '{self.config.name}' failed with return code {process_return_code}.")
                    return {"success": False, "result": None, "error": f"Script execution failed with code {process_return_code}. Stderr: {stderr_str}", "stdout": stdout_str, "stderr": stderr_str}

            except json.JSONDecodeError as je:
                logger.error(f"JSONDecodeError processing script output for '{self.config.name}': {je}")
                return {"success": False, "result": None, "error": f"Failed to decode JSON output from script: {je}"}
            except Exception as e:
                logger.error(f"Exception during Python script sync execution for '{self.config.name}': {e}\n{traceback.format_exc()}")
                return {"success": False, "result": None, "error": f"An unexpected error occurred in sync runner: {str(e)}"}
            finally:
                if tmp_input_file_path and os.path.exists(tmp_input_file_path):
                    try:
                        os.remove(tmp_input_file_path)
                    except OSError as e_remove:
                        logger.warning(f"Could not remove temporary input file {tmp_input_file_path}: {e_remove}")
        ...
        try:
            ...
            logger.error(f"Error invoking asyncio.to_thread for '{self.config.name}': {e_thread}\n{traceback.format_exc()}")
            return {"success": False, "result": None, "error": f"Failed to execute script due to threading error: {str(e_thread)}"}
    ...
    def __del__(self):
        ...
        """
        if hasattr(self, '_script_path_to_execute') and self._script_path_to_execute and \
           self.config.script_content and self._script_path_to_execute in _temporary_script_files:
            try:
                ...
                logger.info(f"Cleaned up temporary script file on MCP instance deletion: {self._script_path_to_execute}")
            except Exception as e:
                logger.error(f"Error cleaning up temporary script {self._script_path_to_execute} during MCP instance __del__: {e}")
