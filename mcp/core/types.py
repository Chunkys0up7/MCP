...
class PythonScriptConfig(BaseMCPConfig):
    ...
    script_path: Optional[str] = Field(
        default=None,
        description="The file system path to the Python script to be executed. Required if script_content is not provided.",
    )
    script_content: Optional[str] = Field(
        default=None,
        description="A string containing the Python script content directly. Required if script_path is not provided. If both are provided, script_content may take precedence depending on MCP implementation.",
    )
    ...
    def check_script_path_or_content_exists(self) -> "PythonScriptConfig":
        if not self.script_path and not self.script_content:
            raise ValueError(
                "Either 'script_path' or 'script_content' must be provided for PythonScriptConfig."
            )
        return self
