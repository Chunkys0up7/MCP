...
def build_parameters_config(tool_index: int) -> Dict[str, Any]:
    ...
    for i in range(param_count):
        ...
        name = st.text_input(
            "Name",
            key=f"param_name_{tool_index}_{i}",
            help="Parameter name"
        )
...
def build_response_format_config() -> Dict[str, Any]:
    ...
    if format_type == "json":
        ...
        schema = st.text_area(
            "JSON Schema",
            help="JSON schema for response format"
        )
        try:
