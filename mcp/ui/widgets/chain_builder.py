...
class ChainBuilder:
    def __init__(self):
        ...
        if 'error_handling' not in st.session_state or st.session_state.error_handling not in valid_error_handling:
        ...
        if 'execution_mode' not in st.session_state or str(st.session_state.execution_mode).lower() not in valid_execution_modes:
    ...
    def render(self):
        ...
        with col1:
            st.session_state.error_handling = st.selectbox(
                ...
                index=["Stop on Error", "Retry with Backoff", "Fallback Chain"].index(st.session_state.error_handling),
            ...
            for mcp in available_mcps:
                ...
                if mcp["type"] == "jupyter_notebook":
                    ...
                    if notebook_path and not Path(notebook_path).exists():
                        st.warning(f"Notebook not found: {notebook_path} (referenced by {mcp['name']})")
        ...
        with st.expander("Chain Configuration", expanded=True):
            ...
            with col1:
                st.session_state.error_handling = st.selectbox(
                    ...
                    index=["Retry with Backoff", "Fallback Chain", "Stop on Error"].index(st.session_state.error_handling)
                )
                
                if st.session_state.error_handling == "Retry with Backoff":
                    st.session_state.max_retries = st.number_input("Max Retries", min_value=1, max_value=5, value=st.session_state.max_retries)
                    st.session_state.backoff_factor = st.number_input("Backoff Factor (seconds)", min_value=1, max_value=10, value=st.session_state.backoff_factor)
    ...
    def _create_chain_config(
        ...
        return {
            ...
            "error_handling": {
                ...
                "max_retries": st.session_state.max_retries if error_handling == "Retry with Backoff" else None,
                "backoff_factor": st.session_state.backoff_factor if error_handling == "Retry with Backoff" else None
            },
