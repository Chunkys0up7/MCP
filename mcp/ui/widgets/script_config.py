...
from mcp.core.types import MCPType, PythonScriptConfig
...
def build_script_config() -> PythonScriptConfig:
    ...
    file_source = st.radio(
        "Script Source", ["Use Existing File", "Create New Script"], key="script_source"
    )

    if file_source == "Use Existing File":
        script_path = st.text_input(
            "Script Path", help="Path to the Python script file", key="script_path"
        )
    else:
        # Create new script
...
""",
    ...
    return PythonScriptConfig(
        ...
        script_path=(
            script_path
            if file_source == "Use Existing File"
            else os.path.join(config.scripts_dir, f"{script_name}.py")
        ),
