...
class DatabaseMonitor:
    ...
    def _get_connection_stats(self) -> Dict[str, int]:
        ...
        row = result.fetchone()
        return {"active": row[0], "idle": row[1], "waiting": row[2]}
    ...
    def _get_transaction_stats(self) -> Dict[str, int]:
        ...
        row = result.fetchone()
        return {"total": row[0], "active": row[1]}
    ...
    def _get_size_stats(self) -> Dict[str, Dict[str, int]]:
        ...
        # Get table sizes
        table_result = self.session.execute(
            text(
                """
            SELECT
        ...
        # Get index sizes
        index_result = self.session.execute(
            text(
                """
            SELECT
        ...
        return {"tables": table_sizes, "indexes": index_sizes}
    ...
    def _check_alerts(self, metrics: DatabaseMetrics) -> None:
        ...
        if metrics.cache_hit_ratio < self.alert_thresholds["cache_hit_ratio"]:
            self._generate_alert(
                "cache_hit_ratio", f"Low cache hit ratio: {metrics.cache_hit_ratio:.2%}"
            )
